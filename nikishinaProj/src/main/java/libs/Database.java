...
public class Database {
   ...
   static Logger log = Logger.getLogger(Database.class);
     ...
     */
    public Database(String db, String driver) throws IOException, ClassNotFoundException, SQLException {    //url k baze i url k driveru
        url=getCfgValue(db);    //method for driver, Mysql for example
        log.info(url);
        // Load driver for JDBC class
        Class.forName(getCfgValue(driver));  //etim metodom my zagruzim nuznij driver (mySQL) v etot class
                
        // Create a connection to the database
        String user_name=getCfgValue((db + "_USER"));    //(db + "_USER") - eto kluch k loginu v baze dannyh
        String user_pass=getCfgValue((db + "_PASSWORD"));     //(db + "_PASSWORD") - kluch k parolu k baze dannyh
        log.info(" user " + user_name + " pass " + user_pass);
        connection= DriverManager.getConnection(url,user_name,user_pass);  //ispolzuem connection, kotorij objavlen vverhu v classe. ustanovi connection k nashej DB, d
                                                                           //kotoryi peredaem url k baze, login i parol
    ...
    public  int changeTable(String query) throws SQLException {
        Statement stm=connection.createStatement(); //sessija, v kotoroj vypolniaetsa zapros
     ...
     *  rezultat zaprosa v dannom metode - ODNO znachenie, naprimer tolko parol
     */
     ...
     *  etot method ispolzuetsa, esli ozhidaem odnu stroku
     */
     ...
     *etot metod vozvraschaet list listov - mnogo strok
     */
...
*/
    ...
    /*
     *  Close connection to the database. Use this method always!
     */
